...
func TestLintMatrixFailsLoudlyWhenJobMissing(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "lint job without strategy",
			yaml: "jobs:\n  lint:\n    runs-on: ubuntu-latest\n",
		},
		{
			name: "lint job without matrix",
			yaml: "jobs:\n  lint:\n    strategy:\n      fail-fast: false\n",
		},
		{
...
func parseLintMatrix(data []byte) ([]matrixEntry, error) {
	...
	if job.Strategy == nil {
		return nil, fmt.Errorf("%s: job %q has no strategy", ciWorkflowRel, lintJobName)
	}
	if job.Strategy.Matrix == nil {
		return nil, fmt.Errorf("%s: job %q has no strategy.matrix", ciWorkflowRel, lintJobName)
	}
	if len(job.Strategy.Matrix.Include) == 0 {
		return nil, fmt.Errorf("%s: job %q strategy.matrix.include is missing or empty", ciWorkflowRel, lintJobName)
	}
