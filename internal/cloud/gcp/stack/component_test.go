...
func TestModuleRegistersRequiredOutputs(t *testing.T) {
	...
	if !found || module.CertificationTier() != platform.TierCertified {
		t.Fatal("expected certified GCP Autopilot module")
	}
	standard := Module{RuntimeID: "gke-standard"}
	if standard.CertificationTier() != platform.TierExperimental {
		t.Fatal("expected experimental GCP Standard module")
	}
}
...
func TestProgramBuildsMockGraph(t *testing.T) {
	spec := Spec{
		...
		Catalog: CatalogSelection{
			CloudSQLTier: "db-perf-optimized-N-2", CloudSQLAvailability: "ZONAL",
...
func TestProgramBuildsStandardPresetGraph(t *testing.T) {
	spec := Spec{
		...
		Policy:      NetworkPolicy{NetworkCIDR: "10.20.0.0/16", Zones: []string{"europe-west1-b", "europe-west1-c"}},
		Catalog: CatalogSelection{
			CloudSQLTier: "db-perf-optimized-N-2", CloudSQLAvailability: "REGIONAL",
