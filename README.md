...
Deploy Magento Open Source and Adobe Commerce in your own AWS or GCP account
with ACC/Upsun-shaped YAML and a CLI. You keep the cloud account. MageLift is
not a hosting service.

> **Status:** first public tag target is `v1.0.0-rc.1`. After that tag,
> [docs/versioning.md](docs/versioning.md) freezes the CLI/YAML contract for the
> RC series. Untagged `main` is not a stable release. Gates live in
...
- typed config with compatibility checks before mutate
...
Each cloud lives under `internal/cloud/<provider>/` behind
`platform.StackModule` ([adding a provider](docs/adding-a-provider.md)).

## Install

Until `v1.0.0-rc.1`, install from source (Releases is empty). Prefer a release
archive after the first tag; see [Install](https://magelift.dev/docs/install/).
...
See [local vs cloud](docs/local-vs-cloud.md) for what local Compose does and does
not match in cloud.
