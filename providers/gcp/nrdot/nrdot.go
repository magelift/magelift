// Package nrdot is a behavior-frozen vendored copy of the NRDOT Helm subset
// from internal/external/newrelic/nrdot.go (forked 2026-09-16 for the
// autonomous GCP provider, which must not import that package's transitive
// closure). Do not evolve here: fix upstream and re-vendor, or promote the
// subset deliberately.
package nrdot
...
// The helpers below are vendored with nrdot.go (same fork note applies):
// isNRDOTKubernetesTarget and validateNewRelicTarget from
// internal/external/newrelic/integration.go, validateOwnershipMarker from
// internal/external/newrelic/otlp.go.
