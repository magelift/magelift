...
The build MUST read Quality Patches IDs from `.magento.env.yaml` at `stage.build.QUALITY_PATCHES`. IDs MUST be validated as non-empty patch identifiers and MUST preserve the configured order. Duplicate IDs or malformed configuration MUST fail before Composer or patch execution.

#### Scenario: Standalone Quality Patches Tool is locked

- **GIVEN** `.magento.env.yaml` contains an ordered `QUALITY_PATCHES` list
...
- **GIVEN** `.magento.env.yaml` contains one or more Quality Patch IDs
...
Local, CI, and cloud preparation MUST use the same patch selection and ordering rules from the source tree and production Composer lock. Environment-specific credentials or deployment settings MUST NOT change whether configured patches are selected, skipped, or duplicated.

#### Scenario: The same locked source is prepared in two environments

- **GIVEN** two preparations receive the same source revision, lock file, and `.magento.env.yaml`
