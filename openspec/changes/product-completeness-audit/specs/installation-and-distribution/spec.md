...
First-party providers MAY ship in the main module. If independent provider modules are released later, they MUST carry their own versions and satisfy `provider-extension-loading`. Community providers MUST NOT be required to live in this repository. Changelog and GitHub release notes MUST list user-visible CLI and YAML changes for the core module.

#### Scenario: A community provider is not in the core tarball

- **WHEN** a user installs only the core MageLift binary
- **THEN** first-party in-tree providers that this release includes are available, and an uninstalled community provider is reported as unregistered rather than silently loaded from the network
