...
docker run -d --name "$backend" --network "$network" docker.io/library/nginx:1.27-alpine >/dev/null
docker exec "$backend" sh -c 'printf "server { listen 80; location = /health { default_type text/plain; return 200 \"OK\\n\"; } location / { return 404; } }\n" > /etc/nginx/conf.d/default.conf && nginx -s reload' >/dev/null
