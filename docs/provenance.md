...
| Magento patch lifecycle | Clean-room PHP build (`PatchLifecycle`, `PatchApplier`) | Public Adobe patch-order documentation plus project-installed upstream executables; no vendored Cloud Patches source or Quality Patches database | Delegate required Cloud Patches, configured Quality Patches, and local hotfix ordering to the target's locked tools; retain an argv-safe idempotent custom fallback | Recorded |
