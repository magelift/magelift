...
paid follow-up gate**; no fourth paid AWS pass in Phase 5.
...
evidence are **paid follow-up gate**. Document the planned records here operationally,
...
### Evidence (MIGRATE-04 / SC5)

| Slice | Status |
| --- | --- |
| Runbook + local scratch (dump, media unit/Floci, maintenance/reindex/verify/rollback command shapes) | Phase 5 |
| Live DNS + full non-prod cutover rehearsal + managed-instance dump cell | paid follow-up gate (no Phase 5 paid AWS pass) |
