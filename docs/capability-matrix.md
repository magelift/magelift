...
| Search | `catalog.searchMode` | `disabled` (preview / free-tier), `serverless` (preview), `provisioned` | Free-tier-safe default is `disabled`. Wiring certified offline + prior Magento-on-AWS OpenSearch ops ([source](sources/chantelle-opensearch.md)); MageLift live SigV4 data-plane deferred to paid acceptance. Do not claim live Magento search on MageLift is green yet |
