// Command ovh-recovery-acceptance runs one disposable OVHcloud Object
// Storage backup, isolated-restore, integrity, and ownership-cleanup cell.
//
// The command keeps S3 credentials at the provider edge. They are read from
// environment variables, passed to the official SDK credential provider, and
// never enter an operation request, operation ID, evidence, or process output.
package main
...
func main() {
	if err := runMain(); err != nil {
		fmt.Fprintf(os.Stderr, "OVHcloud recovery acceptance failed: %v\n", err)
		os.Exit(1)
	}
}
...
func run(parent context.Context, args []string, output io.Writer) (runErr error) {
	flags := flag.NewFlagSet("ovh-recovery-acceptance", flag.ContinueOnError)
	flags.SetOutput(io.Discard)
	region := flags.String("region", "", "OVHcloud Object Storage region, for example gra")
	...
	endpoint := flags.String("endpoint", "", "optional HTTPS S3 endpoint; otherwise use the regional default")
	...
	accessKey := strings.TrimSpace(os.Getenv("MAGELIFT_OVH_S3_ACCESS_KEY"))
	secretKey := strings.TrimSpace(os.Getenv("MAGELIFT_OVH_S3_SECRET_KEY"))
	if accessKey == "" || secretKey == "" {
		return errors.New("MAGELIFT_OVH_S3_ACCESS_KEY and MAGELIFT_OVH_S3_SECRET_KEY are required")
	}
	resolvedEndpoint, err := resolveEndpoint(*endpoint, *region)
	if err != nil {
		return err
	}
	...
	defer cancel()
	credentialProvider := credentials.NewStaticCredentialsProvider(accessKey, secretKey, "")
	...
	if err != nil {
		return fmt.Errorf("load OVHcloud Object Storage SDK configuration: %w", err)
	}
	objects := s3.NewFromConfig(awsConfig, func(options *s3.Options) {
		options.BaseEndpoint = aws.String(resolvedEndpoint)
		options.UsePathStyle = true
	})
	native, err := ovhresilience.NewOVHNativeAPI(ctx, ovhresilience.NativeAPIConfig{
		ArchiveBucket: *bucket, RestoreBucket: *bucket, Region: *region, Endpoint: resolvedEndpoint,
		Credentials: credentialProvider, ArchivePrefix: "magelift/recovery", RestorePrefix: "magelift/recovery/restored",
		RetentionDays: 1, RequireObjectLock: false,
	})
	if err != nil {
		return fmt.Errorf("construct OVHcloud recovery translator: %w", err)
	}
	operationClient, err := ovhresilience.NewNativeResilienceClient(native)
	if err != nil {
		return fmt.Errorf("construct OVHcloud recovery operation client: %w", err)
	}

	sourcePrefix := "fixture/" + *fixture
	resourceReference := "ovh-object://" + *bucket + "/" + sourcePrefix
	cell, err := certification.NewObjectRecoveryCell(certification.ObjectRecoveryCellConfig{
		Operations: operationClient, Store: ovhObjectStore{client: objects}, Bucket: *bucket,
		ResourceReference: resourceReference, Marker: *marker, FixtureID: *fixture, SourcePrefix: sourcePrefix,
	...
	fmt.Fprintf(output, "OVHcloud Object Storage recovery acceptance PASS region=%s bucket=%s objects=%d backup=%s restore=%s fixtureFingerprint=%s cleanup=verified\n", *region, *bucket, result.ObjectCount, result.BackupID, result.RestoreID, result.FixtureFingerprint)
	return nil
}

func resolveEndpoint(explicit, region string) (string, error) {
	endpoint := strings.TrimSpace(explicit)
	if endpoint == "" {
		endpoint = "https://s3." + strings.ToLower(strings.TrimSpace(region)) + ".io.cloud.ovh.net"
	}
	parsed, err := url.Parse(endpoint)
	if err != nil || parsed.Scheme != "https" || parsed.Host == "" || (parsed.Path != "" && parsed.Path != "/") || parsed.RawQuery != "" || parsed.Fragment != "" {
		return "", errors.New("OVHcloud Object Storage endpoint must be an HTTPS URL without a path, query, or fragment")
	}
	return strings.TrimRight(endpoint, "/"), nil
}

type ovhObjectStore struct {
	client *s3.Client
}

func (store ovhObjectStore) Put(ctx context.Context, bucket string, object certification.ObjectRecoveryFixture) error {
...
func (store ovhObjectStore) Delete(ctx context.Context, bucket, key string) error {
	_, err := store.client.DeleteObject(ctx, &s3.DeleteObjectInput{Bucket: aws.String(bucket), Key: aws.String(key)})
	return err
}

func (store ovhObjectStore) ReadPrefix(ctx context.Context, bucket, prefix string) (map[string][]byte, error) {
...
func parseObjectReference(reference string) (bucket, prefix string, err error) {
	...
	if err != nil || parsed.Scheme != "ovh-object" || strings.TrimSpace(parsed.Host) == "" || parsed.RawQuery != "" || parsed.Fragment != "" {
