...
- [ ] 3.2 Implement the net/rpc protocol server plus all 28 operations (lifecycle, bootstrap, state, secrets, observe, tunnel, cost, cleanup, adapter proxies) with per-operation timeouts — verify: `go test ./providers/gcp/...` (module tests) pass with fakes for every operation
