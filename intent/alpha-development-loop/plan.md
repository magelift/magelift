...
- [ ] 11.1 Operator, ask-first: commit this tree, then push `v0.0.0-canary.<40-lowercase-hex-sha>` at that commit — verify: `gh release view` shows a draft whose certificate identity matches `internal/providerhost/lock.go`. Do not push this from an implementation step. Current `HEAD` is `77f3467cd47cb3f441b942dbd3eb10dd00b4a952` and does not contain the lifecycle fix. GoReleaser v2.18.1 does not set `GOWORK=off`, and `go list -m` in `providers/gcp` resolves `github.com/magelift/magelift` to the workspace, so a canary built from a commit of this tree includes the new shell. A tag on current `HEAD` would not.
