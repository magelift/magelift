...
- [ ] 11.1 Operator, ask-first: push `v0.0.0-canary.<full sha>` at the commit that contains this plan and the `setup:upgrade`-before-import change — verify: `gh release view` shows a draft whose certificate identity matches `internal/providerhost/lock.go`. Do not push this from an implementation step. GoReleaser v2.18.1 does not set `GOWORK=off`; the release workflow sets `GOWORK` to the checkout `go.work`, so the provider compiles that commit rather than published rc.22.
